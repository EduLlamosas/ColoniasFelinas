...
describe('Estadisticas (integración real, e2e)', () => {
  ...
  it('agrega gatos por estado CER, comederos sin visita reciente y esterilizaciones del trimestre, todo con datos reales', async () => {
    ...
    await prisma.gato.createMany({
      data: [
        { coloniaId: colonia.id, sexo: 'MACHO', capaPelaje: 'Negro', estadoCer: 'CAPTURADO' },
        { coloniaId: colonia.id, sexo: 'HEMBRA', capaPelaje: 'Blanco', estadoCer: 'CAPTURADO' },
        { coloniaId: colonia.id, sexo: 'MACHO', capaPelaje: 'Gris', estadoCer: 'ESTERILIZADO' },
      ],
    });
    ...
    const comederoReciente = await prisma.comedero.create({
      data: { coloniaId: colonia.id, ubicacionDetallada: 'Comedero con visita reciente' },
    });
    const comederoAntiguo = await prisma.comedero.create({
      data: { coloniaId: colonia.id, ubicacionDetallada: 'Comedero con visita antigua' },
    });
    ...
    const gatoParaIntervencion = await prisma.gato.create({
      data: { coloniaId: colonia.id, sexo: 'MACHO', capaPelaje: 'Atigrado', estadoCer: 'CAPTURADO' },
    });
